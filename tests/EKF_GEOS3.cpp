...
int main() {
    ...
    for (int i = 1; i <= 46; ++i) {
		...
		double x_pole = std::get<0>(iers_result), y_pole = std::get<1>(iers_result);
		double UT1_UTC = std::get<2>(iers_result), LOD = std::get<3>(iers_result);
		double dpsi = std::get<4>(iers_result), deps = std::get<5>(iers_result);
		double dx_pole = std::get<6>(iers_result), dy_pole = std::get<7>(iers_result);
		double TAI_UTC = std::get<8>(iers_result);

		auto time_result = timediff(UT1_UTC, TAI_UTC);
		double UT1_TAI = std::get<0>(time_result), UTC_GPS = std::get<1>(time_result);
		double UT1_GPS = std::get<2>(time_result), TT_UTC = std::get<3>(time_result);
		double GPS_UTC = std::get<4>(time_result);
		...
		double yPhi_norm = yPhi.norm();
		if (yPhi_norm > 1e10 || std::isnan(yPhi_norm)) {
			std::cerr << "Error: yPhi explota en iteración " << i << ", norm: " << yPhi_norm << "\n";
			return 1;
		}
		...
		if (Y_norm > 1e10 || std::isnan(Y_norm)) {
			std::cerr << "Error: Y explota en iteración " << i << ", norm: " << Y_norm << "\n";
			return 1;
		}
		...
		double Azim = std::get<0>(azel_result), Elev = std::get<1>(azel_result);
		Matrix dAds = std::get<2>(azel_result), dEds = std::get<3>(azel_result);
		...
		Matrix K, z(1, 1), g(1, 1), s_sigma(1, 1);
		z(1, 1) = obs(i, 2); g(1, 1) = Azim; s_sigma(1, 1) = sigma_az;
		auto meas_result = MeasUpdate(Y, z, g, s_sigma, dAdY, P, 6);
		K = std::get<0>(meas_result); Y = std::get<1>(meas_result); P = std::get<2>(meas_result);
		...
		Azim = std::get<0>(azel_result); Elev = std::get<1>(azel_result);
		dAds = std::get<2>(azel_result); dEds = std::get<3>(azel_result);
		...
		z(1, 1) = obs(i, 3); g(1, 1) = Elev; s_sigma(1, 1) = sigma_el;
		...
		z(1, 1) = obs(i, 4); g(1, 1) = Dist; s_sigma(1, 1) = sigma_range;
