...
Matrix Accel(double x, Matrix Y) {
    ...
    if (Y.n_row != 6 || Y.n_column != 1) {
        std::cerr << "Accel: Y debe ser 6x1, dims=" << Y.n_row << "x" << Y.n_column << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    std::tie(r_Mercury, r_Venus, r_Earth, r_Mars, r_Jupiter, r_Saturn, r_Uranus, r_Neptune, r_Pluto, r_Moon, r_Sun) = JPL_Eph_DE430(MJD_TDB);
