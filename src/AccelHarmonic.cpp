...
Matrix AccelHarmonic(Matrix& r, Matrix& E, int n_max, int m_max) {
    ...
    for (int n = 0; n <= n_max; ++n) {
        ...
        for (int m = 0; m <= std::min(m_max, n); ++m) {
            q1 += pnm(n + 1, m + 1) * (Cnm(n + 1, m + 1) * std::cos(m * lon) + Snm(n + 1, m + 1) * std::sin(m * lon));
            q2 += dpnm(n + 1, m + 1) * (Cnm(n + 1, m + 1) * std::cos(m * lon) + Snm(n + 1, m + 1) * std::sin(m * lon));
            q3 += m * pnm(n + 1, m + 1) * (Snm(n + 1, m + 1) * std::cos(m * lon) - Cnm(n + 1, m + 1) * std::sin(m * lon));
        }
    ...
    double ax = (1.0 / d * dUdr - r_bf(3, 1) / (d * d * sqrt_r2xy) * dUdlatgc) * r_bf(1, 1) - (1.0 / r2xy * dUdlon) * r_bf(2, 1);
    double ay = (1.0 / d * dUdr - r_bf(3, 1) / (d * d * sqrt_r2xy) * dUdlatgc) * r_bf(2, 1) + (1.0 / r2xy * dUdlon) * r_bf(1, 1);
